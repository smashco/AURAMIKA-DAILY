    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        headerSliverBuilder: (context, innerBoxIsScrolled) => [
          ...
          // ── Vendor Info Card (pinned below banner) ──────────────────────
          SliverToBoxAdapter(
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        // ── Banner background ─────────────────────────────────────────────
        Container(
          decoration: BoxDecoration(color: vendor.brandColor),
...
// ── Vendor Product Grid ───────────────────────────────────────────────────────
class _VendorProductGrid extends StatelessWidget {
  final List<VendorProduct> products;
  final TabController tabController;
  final List<String> tabs;

  const _VendorProductGrid({
    required this.products,
    required this.tabController,
    required this.tabs,
  });
  ...
  Widget build(BuildContext context) {
    if (products.isEmpty) {
      return _EmptyMaterialState(
        material: tabs[tabController.index],
      );
    }

    return CustomScrollView(
      slivers: [
        // ── Material filter summary bar ─────────────────────────────────
        SliverToBoxAdapter(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Stay tuned for exclusive designs',
              style: AppTextStyles.bodySmall.copyWith(
